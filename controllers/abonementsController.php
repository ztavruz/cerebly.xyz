            ...
            if($user = User::getCurrentUser()){
                ...
                if($abonement = Abonement::getById($id)){
                    ...
                    if($abonement->owner == $user->id){
                        ...
                        /**
                         * при изменени абонитмента появлялась ошибка из-за сортировки 
                         * так как функции сортировки искала предыдущую значения интервала 
                         * если изменить поле last_modifed функции будеть оттуда брат началное интервал для сортировки
                         */

                        $startInterval=json_decode($abonement->intervals, false);
                        $startInterval=$startInterval[0];
                        
                        $last_modifed=new DateTime("2018-05-17 $startInterval:00:00");
                        $abonement->last_modifed=$last_modifed->format('Y-m-d H:i:s');
