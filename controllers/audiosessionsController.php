...
class audiosessionsController  extends Controller{
    ...
    function uploadAction()
    {
        ...
        if(!empty($_POST['start_upload_session']))
        {
            ...
            if(!isset($_SESSION['file']))
            {
                ...
                // $filename = array_pop(explode(DIRECTORY_SEPARATOR, $filename)).'.encrypted';
                $filename = $_POST['filename'].'.encrypted';
                $_SESSION['file'] = array(
        ...
        if(isset($_POST['session_end'])) {
            ...
            try{
                ...
                $file->decryptFile(str_replace(".mp3","",$storagePath.$fileName)); // проверка
            } catch (Exception $e) {
    ...
    function listenAction(){
        if(defined("ADMIN")){
            if(!empty($_REQUEST['id'])){
                ...
                return Document::getTemplate('listen-audiosession', $audiosessions);
            }
        }
        return Document::getTemplate("404", array());
            $options = array(
    ...
    function soursAction(){
        if(!empty($_REQUEST['id'])){
            $id = $_REQUEST['id'];
            global $config;
            $id=str_replace('mp3','encrypted',$id);
            $storagePath = $config['audiosession-storage'];
            $myFile = $storagePath.$id;
            $fh = fopen($myFile, 'r');
